  ...
  if (!passHLT)
    ...
    for (int j = 0; j < nDtRechitClusters; j++) { // loggit
      if (                                        // loggit
          ...
          // askDoesOverlapGenMuon_dt(j)  && // loggit askDoesPassID_dt(j) //
          // loggit
      )
        ids.push_back(j); // loggit
    }                     // loggit
...
bool analyzer_objects::askDoesPassID_csc(int index) {
  if ((cscRechitClusterNStation10[index] > 1 &&
      ...
      (cscRechitClusterNStation10[index] == 1 &&
       ...
       fabs(cscRechitClusterEta[index]) < 1.0))
    return true;
  else
    return false;
}
...
bool analyzer_objects::askDoesPassID_dt(int index) { // loggit
  if (                                               // loggit
      (dtRechitClusterNStation10[index] > 1 &&
       fabs(dtRechitClusterEta[index]) < 1.1) || // loggit
      (dtRechitClusterNStation10[index] == 1 &&
       fabs(dtRechitClusterAvgStation10[index]) == 1 &&
       fabs(dtRechitClusterEta[index]) < 0.9) || // loggit
      (dtRechitClusterNStation10[index] == 1 &&
       fabs(dtRechitClusterAvgStation10[index]) == 2 &&
       fabs(dtRechitClusterEta[index]) < 0.8) || // loggit
      (dtRechitClusterNStation10[index] == 1 &&
       fabs(dtRechitClusterAvgStation10[index]) == 3 &&
       fabs(dtRechitClusterEta[index]) < 0.7) || // loggit
      (dtRechitClusterNStation10[index] == 1 &&
       fabs(dtRechitClusterAvgStation10[index]) == 4 &&
       fabs(dtRechitClusterEta[index]) < 0.6)) // loggit
    return true;                               // loggit
  else
    return false; // loggit
}
