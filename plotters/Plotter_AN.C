...
{
  std::cout<<"-------------------------- Started the function"<<std::endl;
  C->cd();
  float lumix = 0.9;
  float lumiy = 0.91;
  float lumifont = 42;
  std::cout<<"-------------------------- Started the function 1"<<std::endl;
...
void Plotter_AN(TString region, bool dolog, TString inpath, TString aversion){
  ...
  variables.clear();
  // variables.push_back("gLLP_ctau");
  // variables.push_back("gLLP_pt");
  ...
  // variables.push_back("dtRechitClusterSize_v");
  // variables.push_back("dtRechitClusterSize_v2");
  ...
  if (variables[i]=="cscRechitClusterSize" || variables[i]=="dtRechitClusterSize") {
   ...
   leg->SetMargin(0.22);
    ...
    if (variables[i].Contains("Bx") && variables[i].Contains("dt")) {
        ...
        h_sig[j]->GetXaxis()->SetTitle("t_{Cluster}");
        h_sig[j]->GetXaxis()->SetRangeUser(-30, 30);
    }
    ...
    // h_bkg->GetXaxis()->SetRangeUser(-60, 60);
    // h_sig->GetXaxis()->SetRangeUser(0, 100);
    // h_bkg->GetXaxis()->SetRangeUser(0, 100);
    if (j != 8) {
        h_sig[j]->SetLineColor(j+2); // kRed);
    } else {
        h_sig[j]->SetLineColor(50);
    }
    ...
    if(dolog) {
        canvas->SaveAs("plotDump/"+aversion+"/"+variables[i]+"_"+region+"_log.pdf");
        canvas->SaveAs("plotDump/"+aversion+"/"+variables[i]+"_"+region+"_log.C");
    }
    else {
        canvas->SaveAs("plotDump/"+aversion+"/"+variables[i]+"_"+region+".pdf");
        canvas->SaveAs("plotDump/"+aversion+"/"+variables[i]+"_"+region+".C");
    }
    canvas->Clear();
